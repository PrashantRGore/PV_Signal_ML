...
class SISATrainer:
    ...
    def train(self, signals_df: pd.DataFrame, n_shards: int = 10) -> Dict[str, Any]:
        ...
        for i in range(n_shards):
            ...
            # Train shard model
            model = xgb.XGBClassifier(random_state=42 + i)
        ...
        return {
            ...
            'n_test': len(X_test),
            'n_shards': n_shards,
            'report': classification_report(y_test, (y_pred_proba > 0.5).astype(int))
        }
    
    def _ensemble_predict_proba(self, X):
        """Ensemble prediction from all shards"""
        predictions = np.array([model.predict_proba(X)[:, 1] for model in self.shard_models])
        return predictions.mean(axis=0)
    
    def unlearn(self, case_id: int, progress_callback: Optional[Callable] = None) -> Dict[str, Any]:
        """
    ...
    def _retrain_shard(self, shard_id: int):
        ...
        if shard_id < len(self.shard_data):
            ...
            model = xgb.XGBClassifier(random_state=42 + shard_id)
